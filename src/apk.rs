...
use std::io::Seek;
use crate::zip2::archive::ZipEntry;
use crate::zip2::archive::ZipArchive;
...
impl Apk {
    pub fn open(path: &str) -> io::Result<Apk> {
        ...
        let mut zip_archive = ZipArchive::open(path)?;
        let mut resources = None;
        {
            if let Some(mut res_file) = zip_archive.by_name("resources.arsc")? {
